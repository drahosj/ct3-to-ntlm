//! Data Encryption Standard ECB Rust implementation.
//! Thanks to Antoni Boucher : https://github.com/antoyo/des-rs
//! I modified a bit the original code for performance in this use-case.
//! # Example
//!
//! ```
//!
...
fn key_to_u64(key: &Vec<u8>) -> u64 {
    if key.len() != 8 {
        panic!("key vector is not 64 bits/8 bytes long");
    }
    let mut result = 0;
    for &part in key {
        result <<= 8;
...
fn des(message: &[u8], subkeys: Vec<u64>, result: &mut Vec<u8>) {
...
/*pub fn encrypt(message: &[u8], key: &Vec<u8>, result: &mut Vec<u8>) {
    let key = key_to_u64(key);
    let subkeys = compute_subkeys(key);
    result.clear();
    des(message, subkeys, result);
}*/

pub fn decrypt(cipher: &[u8], key: &Vec<u8>, result: &mut Vec<u8>) {
    let key = key_to_u64(key);
    let mut subkeys = compute_subkeys(key);
    subkeys.reverse();
    result.clear();
    des(cipher, subkeys, result);
}
